    ...
    for ii in range(len(systems))[start_sys:end_sys]:
        ...
        for jj in range(system["nimgs"]):
            text_file.write("%s %s %s %s %s %s %s \n" %
                    (system["ximg"][jj], system["yimg"][jj],
                    system["mags"][jj], str(1e-3), str(1e-3),
                    system["delay"][jj], str(1e-3), str(1),
                    )
            )
        text_file.close()
        
        # Optimization Prior file
        data_file_name = args["inbase"] + \
                         "/prior_"+str(ii)+".dat"
        text_file = open(data_file_name, "w")
        text_file.write("range lens %s %s %s %s \n" %
                (1, 6,
                 1, 40,
                )
        )
        ...
        os.system(
                "sed -i '6s@.*@zl "+str(system_prior['zl'])+"@' "+new_file_name
        )
        os.system(
                "sed -i '6s@.*@prefix fitH0_"+str(ii)+"@' "+new_file_name
        )
        os.system(
        ...
        os.system(
            "sed -i '21s@.*@set restart = "+args["restart_1"]+"@' "+new_file_name
        )
        fit1_name = args["outbase"]+"/fit1_"+str(ii)
        os.system(
            "sed -i '26s@.*@optimize "+fit1_name+"@' "+new_file_name
        )
        # Define optimization routine
        os.system(
            "sed -i '29s@.*@set restart = "+args["restart_2"]+"@' "+new_file_name
        )
        os.system(
            "sed -i '30s@.*@setlens "+fit1_name+".start@' "+new_file_name
        )
        ...
        os.system(
            "sed -i '46s@.*@readobs_point source_obs_"+str(ii)+".dat"+"@' "+new_file_name
        )
        os.system(
            "sed -i '47s@.*@parprior prior_"+str(ii)+".dat"+"@' "+new_file_name
        )
        SIEPOI_name = args["outbase"]+"/SIE_POI_"+str(ii)
        os.system(
            "sed -i '40s@.*@optimize "+SIEPOI_name+"@' "+new_file_name
        )
